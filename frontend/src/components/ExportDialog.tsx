...
import type { Aspect, CaptionGranularity } from "../types";
...
export default function ExportDialog({ onClose }: { onClose: () => void }) {
  ...
  const [aspect, setAspect] = useState<Aspect>(edl?.aspect ?? "source");
  const [captions, setCaptions] = useState<boolean>(edl?.captions.enabled ?? false);
  const [granularity, setGranularity] = useState<CaptionGranularity>(
    edl?.captions.granularity ?? "segment",
  );
  ...
  const run = () =>
    startExport({ aspect, captions, granularity, quality } as {
      aspect?: string;
      captions?: boolean;
      quality: string;
    });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {status === "running" ? "Exporting…" : "Export video"}
        </button>
